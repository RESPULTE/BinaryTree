...
def test_find_max(filled_bstree: BSTree):
    assert filled_bstree.find_max() == 14


def test_find_min(filled_bstree: BSTree):
    assert filled_bstree.find_min() == 6


@pytest.mark.parametrize('t_val, ex_val', [(8, 9), (100, None), (0, 6)],
                         ids=['normal', '> max', '< min'])
def test_find_gt(filled_bstree: BSTree, t_val, ex_val):
    assert filled_bstree.find_gt(t_val) == ex_val


@pytest.mark.parametrize('t_val, ex_val', [(8, 6), (5, None), (10, 9)],
                         ids=['normal', '< min', '> max'])
def test_find_lt(filled_bstree: BSTree, t_val, ex_val):
    assert filled_bstree.find_lt(t_val) == ex_val


def test_find_ge(filled_bstree: BSTree):
    assert filled_bstree.find_max() == 14


def test_find_le(filled_bstree: BSTree):
    assert filled_bstree.find_min() == 6
...
@pytest.mark.parametrize('index, expected', [(0, 6), (-1, 14), (1, 8), (-2, 12)])
def test_indexing(filled_bstree: BSTree, index: int, expected: int):
    assert filled_bstree[index] == expected
