...
class QuadTree:
    ...
    def __init__(self, bbox: BBox, capacity: int=2, max_depth: int=12, auto_id: bool=False):
        ...
        self.num_quad_node_in_use   = 1
        self.num_entity_node_in_use = 0
    ...
    def clean_up(self) -> None:
        ...
        def traversal_clean_up(node: QuadNode) -> None:
            if node.first_child != -1 and node.in_use and not node.is_leaf:
                if sum(traversal_clean_up(self.all_quad_node[node.first_child + i]) for i in range(4)) == 4:
                    self.set_free_quad_node(self.all_quad_node[node.first_child])
                    if node == self.root: node.first_child = 1

            if node.total_entity == 0 and node.parent_index != -2:                
                return 1

            return 0

        traversal_clean_up(self.root)
    ...
    def set_free_entity_nodes(self, indexed_entity_nodes: List[Tuple[int, QuadEntityNode]]) -> None:
        owner_node = indexed_entity_nodes[0][1].owner_node
        owner_node.total_entity = 0
        owner_node.first_child  = -1
        
        self.num_entity_node_in_use -= len(indexed_entity_nodes)
    ...
    def set_free_quad_node(self, node: Union[QuadNode, QuadEntityNode]) -> None:

        node_index = self.all_quad_node.index(node)
        parent_node = self.all_quad_node[node.parent_index]

        if parent_node.first_child != node_index:
            raise ValueError(f"the liberation should happen with the first child not the '{node_index % 4}' child")

        for i in range(4):
            child_index = parent_node.first_child + i
            self.all_quad_node[child_index].update(
                first_child=self._free_quad_node_index, 
                total_entity=-1, 
                parent_index=-2
                )

        self._free_quad_node_index = node_index
        self.num_quad_node_in_use -= 4
    ...
    def set_branch(self, node):
        ...
        node.first_child = self._free_quad_node_index

        next_free_quad_node_index = self.all_quad_node[node.first_child].first_child

        for i in range(4):
            child_index = self._free_quad_node_index + i
            self.all_quad_node[child_index].update(first_child=-1, total_entity=0, parent_index=node_index)
    ...
    def insert(self, entity_bbox: BBox, entity_id: Optional['UID']=None) -> None:
        ...
        for node, bbox in self.find_leaves(bbox=entity_bbox):
            ...
            if node.total_entity > self.capacity: 
                ...
                entity_nodes = self.find_entity_node(qnode=node, index=True)
